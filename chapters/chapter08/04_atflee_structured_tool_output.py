"""
Chapter 8-4: Claude 구조화 출력 실습
Tool Use 결과와 Claude 최종 답변을 업무 시스템에서 재사용 가능한 JSON으로 정리한다.

[8-3 vs 8-4 차이]
8-3: Agent Loop로 여러 도구를 연속 사용하고, 최종 답변을 자연어 문장으로 출력한다.
8-4: Agent Loop는 동일하게 유지하되, Claude에게 최종 답변을 JSON만 출력하도록 지시한다.
     json.loads()로 파싱해 필드별로 구조화된 결과를 얻는다.
     → AX Console, VOC Agent, 업무 자동화 시스템에서 그대로 가져다 쓸 수 있다.
"""
...
# 도구(Tool) 정의
# ==============================
# 8-3과 동일한 세 가지 도구를 유지한다.
# 8-4에서 바뀌는 것은 도구 정의가 아니라, 최종 답변 형식(JSON)이다.
...
# ==============================
# 추가 함수 1: clean_json_text
# ==============================
# Claude가 JSON을 ```json ... ``` 코드블록으로 감싸서 반환할 때가 있다.
# json.loads()는 코드블록 마커를 이해하지 못하므로 사전에 제거해야 한다.

def clean_json_text(text):
    """
    Claude 응답에서 JSON 코드블록 마커(```json, ```)를 제거한다.
    json.loads()에 넣기 전에 반드시 이 함수를 거쳐야 한다.
    """
    cleaned = text.strip()
...
# ==============================
# 추가 함수 2: extract_text_from_response
# ==============================
# Claude 응답 content는 여러 블록(text, tool_use 등)으로 구성된다.
# 최종 답변(text 블록)만 추출한다.
...
# ==============================
# 추가 함수 3: print_structured_result
# ==============================
# JSON 파싱에 성공한 결과를 필드별로 보기 좋게 출력한다.
...
# ==============================
# 8-4의 핵심 변경점:
# "Final Output Rules" 섹션에서 Claude에게 JSON만 출력하도록 명시한다.
# 자연어 설명, 마크다운 헤더, 코드블록 설명을 섞지 않게 강하게 지시한다.
...
# ==============================
# Agent Loop + 구조화 출력 함수
# ==============================
# 8-3의 run_agent_loop와 흐름이 동일하다.
# 달라진 점: tool_use가 없을 때 자연어 출력 대신 JSON 파싱을 시도한다.
...
def run_structured_agent(user_question, max_tool_rounds=5):
    ...
    흐름:
    1. 사용자 질문을 전달하고 Claude 응답을 받는다.
    2. tool_use가 있으면 도구를 실행하고 결과를 누적한다.
    3. tool_use가 없으면 최종 답변으로 보고 JSON 파싱을 시도한다.
    4. 파싱 성공 → 구조화 결과 출력 후 dict 반환
    5. 파싱 실패 → 원문과 오류 메시지 출력 후 None 반환
    """
    print("=" * 60)
    print(f"사용자 질문:\n{user_question.strip()}")
    print("=" * 60)
    ...
    for round_index in range(max_tool_rounds):
        ...
        # Claude 응답을 대화 기록에 추가한다.
        messages.append({"role": "assistant", "content": response.content})

        # 응답에서 tool_use 블록을 추출한다.
        tool_use_blocks = [
        ...
        if not tool_use_blocks:
            ...
            # 코드블록 마커가 있으면 제거한 뒤 파싱한다.
            cleaned = clean_json_text(final_text)

            try:
                result = json.loads(cleaned)
                # used_tools는 Python이 직접 추적한 목록으로 덮어쓴다.
                # Claude가 JSON에 기록한 used_tools보다 더 정확하다.
